...
function GameUI.yesOrNo(text, yesCallback, noCallback)
    ...
    background:setBackgroundResource(255,0,0,0,0,"Fk2ztiR-hKdBug6TWtytWvAGu3mr")
    local info = GameUI.mYesOrNo:createUI("Text", "RichMan/GameUI/YesOrNo/Text", "_lt", 0, 0, 600, 90, background)
    info:setTextFormat(5)
    info:setFontSize(25)
    info:setText(text)
    info:setFontColour("255 255 255")
    local button_yes =
        GameUI.mYesOrNo:createUI("Button", "RichMan/GameUI/YesOrNo/Button/Yes", "_lt", 200, 300, 100, 100, background)
    ...
    local button_no =
        GameUI.mYesOrNo:createUI("Button", "RichMan/GameUI/YesOrNo/Button/No", "_lt", 400, 300, 100, 100, background)
...
function GameUI.showBankWindow(callback)
    ...
    background:setBackgroundResource(255,0,0,0,0,"Fk2ztiR-hKdBug6TWtytWvAGu3mr")
    local text_info = GameUI.mBank:createUI("Text", "RichMan/GameUI/Bank/Text_Info", "_lt", 0, 0, 600, 90, background)
    text_info:setFontSize(25)
    ...
    --     end
    -- )
    local text_money = GameUI.mBank:createUI("Text", "RichMan/GameUI/Bank/Text/Money", "_lt", 0, 250, 300, 50)
    text_money:setFontSize(50)
    text_money:setTextFormat(1)
    text_money:setText("请输入金额:")
    local edit = GameUI.mBank:createUI("Edit", "RichMan/GameUI/Edit", "_lt", 300, 250, 300, 50)
    edit:setFontSize(50)
    local button_yes =
...
function GameHost:selectWinner()
    ...
    for _, player in pairs(left_players) do
        if not winner then
            ...
            elseif self.mGameConfig.mSuccessCondition == 3 then
                ...
                mExecuteCallback = function(command)
                    ...
                    for y = 0, 4 do
                        for x = 0, 5 do
                            ...
                            button:setText(tostring(index) .. "万")
                            button:setFontSize(25)
                            button:setFontColour("255 255 255")
                            ...
                            button:addEventFunction(
                                ...
                                function()
                                    GameUI.yesOrNo(
                                        "现金设置为：" .. tostring(index) .. "万，可以吗？",
                                        function()
                                            MiniGameUISystem.destroyWindow(window)
                                            self.mConfig.mMoney = index * 10000
                                            command.mState = Command.EState.Finish
                                        end
                                    )
                                end
                    ...
                    for y = 0, 4 do
                        for x = 0, 5 do
                            ...
                            button:addEventFunction(
                                ...
                                function()
                                    GameUI.yesOrNo(
                                        "存款设置为：" .. tostring(index) .. "万，可以吗？",
                                        function()
                                            MiniGameUISystem.destroyWindow(window)
                                            self.mConfig.mMoneyStore = index * 10000
                                            command.mState = Command.EState.Finish
                                        end
                                    )
                                end
...
function GameManagerClient:start()
    GameUI.showPrepareWindow()
    GameUI.selectWindow({
        {mResource={hash="FkRyA-p9RwB8c8gkRKX5Um-zT2B0",pid="5203",ext="png",},mOnClick=function()
            self.mCommandQueue:post(
                new(
                    Command_Callback,
                    {
...
function Command_CheckRoadClient:execute()
    ...
    elseif block_id == GameConfig.mWalkBlockIDs.mDestiny then
        ...
        if destiny_index == 1 then
            local lost_money = math.random(1000, 10000)
        ...
        elseif destiny_index == 2 then
            message_text = "变卖所有卡片"
            local points = 0
            for _, card in pairs(self.mPlayer:getPropertyCache().mCards) do
                points = points + math.floor(GameConfig.mCards[card].mPrice / 2)
            end
            self.mPlayer:getProperty():safeWrite("mCards", {})
            self.mPlayer:getProperty():safeWrite("mPoint", self.mPlayer:getPropertyCache().mPoint + points)
        elseif destiny_index == 3 then
            local day = math.random(1, 9)
